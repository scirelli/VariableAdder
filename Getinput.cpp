...
char * Get_Input()
{
	...
	switch(Menu())
	{
	case 1 : infile.open("c:\\mydocu~1\\visual~1\\variab~1\\code.txt", ios::in); 
			 strcpy(filename,"c:\\mydocu~1\\visual~1\\variab~1\\code.txt");
	...
	char j;                     //arbitrery variable just there to hold a char, so pointer can more
	...
	char * k = new char[i];           //ok the reason that this string has to be as big as the main on is because the way i take the input using infile>> it reads only chars and stops at spaces, so if there are no spaces in the program the first string is the whole program
